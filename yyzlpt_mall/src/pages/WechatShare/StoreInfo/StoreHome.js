...
class StoreHome extends PureComponent {
  ...
  render() {
    ...
    const bannerList = []
    StoreInfo ? StoreInfo.homeFileVo ? bannerList.push(StoreInfo.homeFileVo) : bannerList.push({id: 'xxx', path: BannerrImg}) : ''
    StoreInfo ? bannerList.push() : ''

    StoreInfo ? StoreInfo.pharmacyVo ? StoreInfo.pharmacyVo.imageIdList.length > 0 ?
      StoreInfo.pharmacyVo.imageIdList.map(item => bannerList.push(item)) : '' : '' : ''

    StoreInfo ? StoreInfo.hospitalVo ? StoreInfo.hospitalVo.hospitalImageList.length > 0 ?
      StoreInfo.hospitalVo.hospitalImageList.map(item => bannerList.push(item)) : '' : '' : ''

    StoreInfo ? StoreInfo.medicalVo ? StoreInfo.medicalVo.medicalImageList.length > 0 ?
      StoreInfo.medicalVo.medicalImageList.map(item => bannerList.push(item)) : '' : '' : ''
    const distanceNum = StoreInfo ? StoreInfo.distance > 1000 ? StoreInfo.distance / 1000 : StoreInfo.distance : 0
    ...
    return (
      <div className={styles.body}>
        <div className={styles.banner}>
          ...
          <span className={styles.CarouselNum}>{this.state.bannerIndex + 1}/{bannerList.length}</span>
          <div className={styles.top}>
            <div className={styles.title} onClick={() => this.toStoreInfo(StoreInfo ? StoreInfo.id : '')}>
              ...
              <div className={styles.addressinfo} onClick={this.toCMap}>
                {StoreInfo ? StoreInfo.distance ?
                  <div>
                    ...
                    <span>距您{distanceNum.toFixed(2)}{StoreInfo.distance > 1000 ? 'km' : 'm'}</span>
                  </div>
                  :
                  <p className={styles.noAddress}>{StoreInfo ? StoreInfo.address : ''}</p>
                  : ''}
              </div>
              {StoreInfo ? StoreInfo.linkPhone ?
                // <a href={'tel:' + StoreInfo.linkPhone}>
                <div className={styles.phone} onClick={() => this.callPhone(StoreInfo.linkPhone)}>
        ...
        <div className={listStyle.listTitleT}>
          <span>热门商品</span><b onClick={() => this.toProductList()}>查看全部商品</b>
        </div>
        <div className={listStyle.serverList}>
          <ul>
            {
              AppHotOffers ? AppHotOffers.length > 0 ?
                AppHotOffers.map(itme => {
                  const marketingPrice = itme.marketingPrice ? itme.marketingPrice / 100 : 0
                  return (<li onClick={() => this.toProductInfo(itme.id,itme.offerName)} key={itme.id}>
        ...
        {
          serviceName.length > 1 ? <div className={listStyle.listTitleT}>
            <span>开通服务</span>
          </div> : ''
        }
        {
          serviceName.length > 1 ? <div className={styles.serverNav}>
            <ul>

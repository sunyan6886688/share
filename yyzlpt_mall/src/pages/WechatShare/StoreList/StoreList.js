...
class StoreList extends PureComponent {
  ...
  componentDidMount = () => {
    ...
    this.setState({
      ...
      labelName: localStorage.getItem('labelName') === '商家列表' ? '全部' : localStorage.getItem('labelName'),
  ...
  getStoreList = (isLoadMore) => {
    ...
    return dispatch({
      ...
      payload: {
        "areaCode": this.state.areaCode != 'null' && this.state.areaCode != 'undefined' ? this.state.areaCode : '',
    ...
    return (
      <div className={styles.tab} ref={ref => { this.fixedTab = ref }}>
        {listTab.map((item, index) => {
          ...
          return (
            <div key={item+index} className={tabBox + ' ' + tabBoxActive} onClick={() => this.chooseBoxTwo(index + 1)}>
              <i>{item}</i><span ></span>
            </div>)
        })}
      </div>
  ...
  renderPicker() {
    ...
    return (
      <Model showModel={tabShow && tabNUm !== 0}
        top={this.fixedTabTop}
        useAnimate={tabNUm !== 0}
        maskClose={() => { this.chooseBoxTwo(0) }}>
                ...
                <ul>
                  {
                    areaList ?
                      areaList.map(item => {
                        ...
                        return (
                          <li key={item.areaCode} onClick={() => this.chooseCity(item)} className={isSelect} key={item.areaCode}>{item.areaName} </li>
                        )
                      })
                      : ''
                  }
                ...
                <ul>

                  {
                    this.state.labelName == '全部' ?
                      <li className={styles.choose} onClick={() => this.chooselable()}>全部</li>
                    ...
                    lableList ? lableList.length > 0 ?
                      lableList.map(item => {
                        ...
                        if (labelName == this.state.labelName) {
                          return (
                            <li key={labelName} onClick={() => this.chooselable(item)} className={styles.choose}>{labelName} </li>
                          )
                        } else {
                          return (
                            <li key={labelName} onClick={() => this.chooselable(item)}>{labelName} </li>
                          )
                        }
            ...
            this.state.tabNUm == 3 ?
              <div className={styles.sortTypeBox}>
                <ul>
                  ...
                  {
                    sortTypeList ? sortTypeList.length > 0 ?
                      sortTypeList.map(item => {
                        const sortTypeName = item.sortTypeName
                        if (sortTypeName == this.state.sortTypeName) {
                          return (
                            <li key={sortTypeName} onClick={() => this.chooseSortType(item)} className={styles.choose}>{sortTypeName} </li>
                          )
                        } else {
                          return (
                            <li key={sortTypeName} onClick={() => this.chooseSortType(item)}>{sortTypeName} </li>
                          )
                        }
    ...
    return (
      <ViewBox header={this.renderHeader()}>
        <Scroll className={styles.scroll}
          ...
          <div ref={ref => { this.hosStore = ref }}>
            {HotStoreList ? HotStoreList.length > 0 ?
              <div className={listStyle.listTitle} >
                <span>人气商家</span><b onClick={this.toRankingList}>查看榜单</b>
              </div>
              : '' : ''}
            {HotStoreList ? HotStoreList.length > 0 ?
              <div className={`${styles.hotStoreList} originEvent`}>
                <ul  >
                  {HotStoreList ? HotStoreList.length > 0 ?
                    ...
                    : '' : ''}
                  {HotStoreList ? HotStoreList.length > 2 ?
                    <li style={{ marginRight: ".24rem", width: '1px' }}><div style={{ width: '1px' }}></div></li>
                    : '' : ''}
                </ul>
              </div>
              : '' : ''}
                      ...
                      <li onClick={() => this.toStoreHome(item.storeId)} key={item.storeId}>
                        ...
                        <div className={listStyle.liRight}>
                          ...
                          {item.hotGoodsName ? <div className={listStyle.dec} onClick={(e) => {
                            ...
                            <p>{item.hotGoodsLabels ? item.hotGoodsLabels.length > 0 ? '[' + item.hotGoodsLabels[0].tagName + "]" : '' : ''}{item.hotGoodsName}</p>
                          </div> : ''
                          }

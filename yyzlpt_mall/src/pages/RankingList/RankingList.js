...
class ShopList extends PureComponent {
  ...
  render() {
    ...
    storeList
      ? storeList.map(item => {
          allStoreListStr.indexOf(JSON.stringify(item)) < 0 ? allStoreList.push(item) : '';
        })
      : '';
    return (
      <Scroll
      ...
      >
        <div className={styles.body}>
          <p className={styles.tips}>
            排行榜基于浙江健康导航App用户大数据，经算法自动计算，每日更新，展示商户排名。
          </p>
              ...
              {allStoreList.length > 0
                ? allStoreList.map((item, index) => {
                    ...
                    return (
                      <li
                        ref="test"
                        onClick={() => this.toStoreHome(item.storeId)}
                        key={item.storeId}
                      >
                        <div
                          className={listStyle.liLeft}
                          style={{
                        ...
                        <div className={listStyle.liRight}>
                          ...
                          <div className={listStyle.liPrice}>
                            ...
                            {item.pricePerPerson > 0 ? (
                              item.distance ? (
                                <b>
                                  ¥{pricePerPerson > 0 ? pricePerPerson.toFixed(2) : ''}
                                  /人
                                </b>
                              ) : (
